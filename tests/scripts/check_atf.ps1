...
Get-ChildItem "$expectedDir\*.atf" | ForEach-Object {
    $name = $_.Name
    $expected = $_.FullName
    $actual = Join-Path $outDir $name

    Write-Host "checking ATF $name"
    ...
    if ($diff) {
        Write-Host "FAIL: ATF differs for $name"
        $fail = $true
    }
}

if ($fail) { exit 1 } else { Write-Host "ATF checks passed" }